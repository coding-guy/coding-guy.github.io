 ...
 */
                (function( window ) {
                    ...
                    var i, support, Expr, getText, isXML, compile, outermostContext, sortInput, hasDuplicate,
                        ...
                        expando = "sizzle" + -(new Date()), preferredDoc = window.document, dirruns = 0,
                        done                                                                        = 0, classCache                                                        = createCache(), tokenCache = createCache(),
                        compilerCache                                                               = createCache(), sortOrder = function( a, b ) {
                            if( a === b ) {
                                hasDuplicate = true;
                            }
                            return 0;
                        ...
                        push                                                                        = arr.push, slice = arr.slice, // Use a stripped-down indexOf if we can't use a native one
                        ...
                        var bySet = setMatchers.length > 0, byElement = elementMatchers.length > 0,
                            superMatcher                              = function( seed, context, xml, results, outermost ) {
                                var elem, j, matcher, matchedCount = 0, i = "0", unmatched = seed && [],
                                    setMatched                                             = [], contextBackup = outermostContext, // We must always have either seed elements or outermost context
...
// Support: Firefox, Chrome, Safari
    ...
    var rxhtmlTag                                                   = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi,
        rtagName                                                    = /<([\w:]+)/, rhtml = /<|&#?\w+;/, rnoInnerhtml = /<(?:script|style|link)/i, // checked="checked" or checked
...
// Called only from within defaultDisplay
    ...
    (function() {
        var pixelPositionVal, boxSizingReliableVal, // Support: Firefox, Android 2.3 (Prefixed box-sizing versions).
            ...
            docElem = document.documentElement, container = document.createElement( "div" ),
